...
Public Class New_Admission_by_term
    ...
    Protected Sub GetReports()
        ...
        rptReportMain.DataSource = GetData("SELECT myyear FROM new_learners_v group by myyear")
    ...
    Protected Sub GetReportsByDate()
        ...
        rptReportMain.DataSource = GetData("SELECT myyear FROM new_learners_v where myyear = '" & dlpyears.Text & "' group by myyear")
        rptReportMain.DataBind()
    ...
    Protected Sub Getyear_date()
        ...
        Using con As SqlConnection = New SqlConnection(sCon)
            Using cmd As SqlCommand = New SqlCommand("select myyear from new_learners_v group by myyear")
    ...
    Protected Sub GetTotalCost()
        ...
        Try
            ...
            For Each item As RepeaterItem In rptReportMain.Items
                ...
                Dim connetionString As String = sCon
                Dim cnn As SqlConnection = New SqlConnection(connetionString)
                Dim cmd As New SqlCommand() With {.CommandType = CommandType.Text, .CommandText = "select sum(amt_fee_paid) as amt_fee_paid,sum(amt_admi_paid) as amt_admi_paid from new_learners_v where myyear = @myyear"}
                'insert product
                cmd.Parameters.Add("@myyear", SqlDbType.VarChar).Value = lblproduct.Text
                cmd.Connection = cnn
                cnn.Open()
    ...
    Protected Sub OnItemDataBound(sender As Object, e As RepeaterItemEventArgs)
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            rptRepoDetails.DataSource = GetData(String.Format("SELECT student_name, tuition_fee, amt_fee_paid, admission_fee, amt_admi_paid, gender,class_name FROM new_learners_v WHERE myyear ='{0}'", supdate))
